            ...
            <div className="container flex justify-between mx-auto">
                <div className="flex items-center md:space-x-3">
                    {(user?.webUser.group.isStaff) &&
                        <Link href="/staff" passHref={true}><a><UserbarLink title="Staff Panel"
                                                                            icon={<AiOutlineDashboard/>}/></a></Link>}
                </div>
                {loading ? (
                    <div className="flex items-center md:space-x-2">
                        <UserbarLink title="Logging in..."
                                     iconRight={<AiOutlineLoading3Quarters className="animate-spin"/>}/>
                    </div>
                ) : (
                    <div>
                        {user ? (
